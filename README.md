...
PowerShell module for the [Cove Data Protection](https://www.n-able.com/products/cove-data-protection) API.

Provides authentication, device enumeration, per-device error and session queries, and a parallel execution engine that handles visa distribution to thread jobs automatically.
...
Sends a generic JSON-RPC request to the main API endpoint. Use this for methods not covered by a dedicated function.

```powershell
$resp = Invoke-CoveJsonrpc -Method 'GetPartnerInfo' -Params @{ partnerId = Get-CovePartnerId }
```
...
Runs a script block against each item in parallel. The module state (visa, partner ID, API URLs) is pre-initialized in each thread job - any module function can be called directly from within the script block without passing credentials.
...
All per-device functions accept an optional `-Visa` parameter (defaults to the stored visa) and an optional `-Endpoint` parameter. When `-Endpoint` is omitted the function calls `Get-CoveAccountInfo` internally. Pass a pre-fetched endpoint to avoid the extra API call when making multiple queries for the same device.
...
Returns `Name`, `Token`, and `RepservUrl` for a device. Returns `$null` if the account cannot be resolved.
...
Returns M365 error history across Exchange, OneDrive, SharePoint, and Teams via the reporting API.
...
Return shape is identical to `Get-CoveDeviceErrors`. `History` is always empty (M365 reporting API returns current session only).
...
Returns session history for a device. Each session is mapped to a normalized status code.
